...
| **reference-scraper**   | "CECC Webscraper": Crawlee + Playwright + Drizzle. **No Effect.** Overwrites history each run; stores no raw markdown; `course_data` mostly `{}`                                                                                                             | `src/dates.ts` (tested chrono-node range parser — port verbatim), `src/utils.ts` (sha256 `generateHash`), the **per-site CSS selectors** in the 6 scrapers (hard-won DOM knowledge, esp. `cpe-`/`cgs-`), and confirmation that its `cecc_course_index_course_listing` table matches §5.1 field-for-field                                                                           | Playwright (ADR-002 rejects a browser), Crawlee, Drizzle, the NextAuth scaffolding, Firecrawl (unused dep), the Excel/email report path, committed `storage/` queue files |
| **reference-catalog**            | "edu2go-reporter": Effect **v4 beta** (`4.0.0-beta.66`) in the _service_ layer only; Bun; HTTP/DB are hand-rolled (non-Effect)                                                                                                                               | The v4 idioms: `Context.Service` + `static Default` layer, `Config.all`/`Config.redacted`, `Data.TaggedError` + `Match.typeTags`, `Effect.forEach({concurrency})`, `Schedule.jittered(both(recurs,exponential))` + `Effect.retry`, `Effect.acquireRelease`+`Scope`, `Queue`-backed worker pool, `pg.Pool` hardening                                                                | Its Bun.serve router and raw-`pg` DB layer (we use `effect/unstable/httpapi` + `@effect/sql-pg` instead)                                                                  |
| **reference-ai-chat** | A **complete** Effect v4 (`4.0.0-beta.88`) chat app: pnpm monorepo (domain/server/client), first-party `effect/unstable/ai`, `Workflow`-per-run, PubSub-mailbox streaming, `@effect/sql-pg` + `Model.Class` + `SqlSchema`, `@effect/vitest` + testcontainers | **The full-stack template.** The agent loop, tool definitions, provider-as-swappable-layer, `Model.Class`/`SqlSchema`/migrator DB layer, mock-`LanguageModel` test harness, DB-enforced single-active-run, prompt builder as the RAG injection seam                                                                                                                                | Its RPC-over-WebSocket transport (we use HttpApi + SSE per §10.3/§10.5 — but the _internals_ port unchanged)                                                              |
| **reference-ai-lib**             | `@reference-ai-lib/core`: Effect v4 (`4.0.0-beta.94`) **provider-neutral AI primitives** (not a framework). pnpm monorepo, port/adapter per vendor                                                                                                                 | Patterns: **dual-tag provider layer** (register vendor tag + generic `LanguageModel` tag over one impl), canonical `TurnEvent` union, `StructuredFormat` + `Turn.decodeStructured`, Standard-Schema tools, **subset-aware `Retry`**, typed `AiError` taxonomy, tool _kinds_ (`signal`/`interaction` for refusal/HITL), `EmbeddingModel` + `jina` adapter, AI-SDK UI-message bridge | Nothing structural — it's a menu of patterns and an optional adapter (§8.6)                                                                                               |
