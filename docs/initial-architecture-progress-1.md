...
|                      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Phase**            | 7 — History — **DONE & RUN**. `course_history` (§5.3.5 rollup + change log) + observation-window honesty (§10.6) as a **deterministic** pure function; router **routes** temporal → history (no longer refuses); typed `history` SSE event; **synthetic-history** testing double (guarded, test/scratch-only). Both branches verified live on the real corpus (grounded multi-term + honest "I don't know yet"); gate still green (filter_exact **100%**, nDCG **0.99**) |
| **Milestone (§16)**  | M0 ✅ · M1 ✅ · M2 ✅ · M3 ✅ · M4 ✅ · M5 ✅ · M6 ✅ · **M7 ✅** (`course_history` + §10.6 honesty + `temporal` eval slice + synthetic-history harness). Next: M8 Ablate (§11.5 table by query shape; reranker keep/drop; ADR-004 crossover)                                                                                                                                                                                                                            |
| **Repo**             | git repo (`main`) + pnpm workspace (**3 packages**: `domain`, `server`, `apps/web`); effect **`4.0.0-beta.99`** + **Astro 5**; migrations `0001`–`0006`; **140 tests green** (`tsc`·`lint`·`dprint`·`vitest` + `astro check`). Phase 7: `domain/history.ts`, `retrieval/course-history.ts`, `history/{format-history,synth-history,load-synth}.ts`, `main-synth-history.ts`, router-v4 + agent history branch                                                            |
...
| **Immediate gate**   | Phase 7 exit met — temporal questions answered honestly against the observation window: grounded multi-term answers on real data ("fee risen $999→$1099 … only what I've observed since July 2026") and honest "I've only been watching since July 2026" at n=1, both `refused:false`. Synthetic-history harness guarded (refuses without `ALLOW_SYNTHETIC_HISTORY=1`). Next: **Phase 8 (Ablate)** — §11.5 table by query shape; reranker keep/drop                      |
